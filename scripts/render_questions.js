...
function renderQuestions() {
  ...
  for (let i = 0; i < numCategories; i++) {
    let questionCells = "";
    for (var questionVal of questionValues) {
      var questionCell = `<div class='question-cell' data-category='${i}'>${
        "$" + questionVal
      }</div>`;
      questionCells += questionCell;
    }
    var questionColumn = `<div class='questions-column'>${questionCells}</div>`;
    $("#questions").append(questionColumn);
  }
}
